...
import com.keyu.system.service.ISysPostService;
...
public class SysUserController extends BaseController
{
    ...
    @PostMapping("/export")
    public void export(HttpServletResponse response, SysUser user, String ids)
    {
        // 若勾选，则只查勾选数据，否则查全部
    ...
    public void importTemplate(HttpServletResponse response) throws Exception {
        // 查询部门姓名id,手动set @excel属性值（combo,readConverterExp）修改excel模板部门为下拉列表
        List<SysDept> list = sysDeptMapper.selectDeptNameAndId();
        List<String> collect = list.stream().map(SysDept::getDeptName).collect(Collectors.toList());
        StringBuilder res = new StringBuilder();
        for (SysDept sysDept : list) {
            res.append(sysDept.getDeptId()).append("=").append(sysDept.getDeptName()).append(",");
        }
        String str = res.substring(0, res.length() - 1);
        ExcelUtil.setCombo("deptId", collect);
        ExcelUtil.setReadConverterExp("deptId", str);
        ExcelUtil<SysUser> util = new ExcelUtil<SysUser>(SysUser.class);
